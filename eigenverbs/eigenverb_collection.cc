...
void eigenverb_collection::add_eigenverb(eigenverb_model::csptr verb,
  ...
  eigenverb_collection::point center(verb->position.latitude(),
                                     verb->position.latitude());
  _collection[interface].insert(
      eigenverb_collection::pair(center, eigenverb_model::csptr(verb)));
}
...
eigenverb_list eigenverb_collection::find_eigenverbs(
    const eigenverb_collection::box& box, size_t interface) const {
  read_lock_guard guard(_mutex);

  // find eigenverbs whose bounding_box intersects this box

  std::list<eigenverb_collection::pair> pair_list;
  _collection[interface].query(bgi::intersects(box),
...
void eigenverb_collection::write_netcdf(const char* filename,
  ...
  if (!list.empty()) {
    ...
    NcVar* east_var = nc_file.add_var("bounding_east", ncDouble, eigenverb_dim);
    NcVar* west_var = nc_file.add_var("bounding_west", ncDouble, eigenverb_dim);
...
void eigenverb_collection::read_netcdf(const char* filename, size_t interface) {
  ...
  NcVar* north_var = nc_file.get_var("bounding_north");
  NcVar* south_var = nc_file.get_var("bounding_south");
  NcVar* east_var = nc_file.get_var("bounding_east");
  NcVar* west_var = nc_file.get_var("bounding_west");
  ...
  for (int rec = 0; rec < num_eigenverbs; ++rec) {
    ...
    north_var->set_cur(rec);
    south_var->set_cur(rec);
    east_var->set_cur(rec);
    west_var->set_cur(rec);
    ++rec;
    ...
    // clang-format off
        ...
        north_var->get(&v, 1);          verb->bounding_box.north = v;
        south_var->get(&v, 1);          verb->bounding_box.south = v;
        east_var->get(&v, 1);           verb->bounding_box.east = v;
        west_var->get(&v, 1);           verb->bounding_box.west = v;
    // clang-format on
