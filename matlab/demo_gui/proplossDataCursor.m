function dcmH = proplossDataCursor(h, datalabels)
...
%             'number of labels is different than the number of data points in the line'])
    ...
    zvals = get(get(event_obj,'Target'),'ZData') ;
    % now figure out which data point was selected
    datapoint = find( (xvals==pos(1))&(yvals==pos(2))&(zvals==pos(3)) ) ;
    label_string = labels{datapoint} ;
    output_txt = { label_string; ...
        ['Lat: ',num2str(pos(2),4)]; ...
        ['Lon: ',num2str(pos(1),4)]; ...
        ['Alt: ',num2str(pos(3),4)]; } ;
    end
