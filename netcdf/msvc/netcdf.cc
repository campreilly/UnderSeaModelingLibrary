...
{                                                                             \
    if (! the_file->define_mode())                                            \
      return FALSE;                                                           \
    if (NcError::set_err(                                                     \
        makename2(nc_put_att_,TYPE) (the_file->id(), the_id, aname, (nc_type) NcTypeEnum(TYPE), 1, &val) \
            ) != NC_NOERR)                                                    \
      return FALSE;                                                           \
    return TRUE;                                                              \
}
...
{                                                                         \
    if (! the_file->define_mode())                                        \
      return FALSE;                                                       \
    if (NcError::set_err(                                                 \
        makename2(nc_put_att_,TYPE) (the_file->id(), the_id, aname, (nc_type) NcTypeEnum(TYPE), len, vals) \
                ) != NC_NOERR)                                            \
      return FALSE;                                                       \
    return TRUE;                                                          \
}
