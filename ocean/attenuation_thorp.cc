...
void attenuation_thorp::attenuation(
    ...
    // reinitialize if frequency values have changed

    if (prev_freq != &frequencies) {
        att_coeff.resize( location.size1(), location.size2() ) ;
        for (unsigned row = 0; row < location.size1(); ++row) {
            for (unsigned col = 0; col < location.size2(); ++col) {
                att_coeff(row, col).resize(frequencies.size());
                for (unsigned f = 0; f < frequencies.size(); ++f) {
                    double F2 = frequencies(f);
                    F2 = 1e-6 * F2 * F2;
                    att_coeff(row, col)(f) = 1e-3 *
    ...
    for (unsigned row = 0; row < location.size1(); ++row) {
        for (unsigned col = 0; col < location.size2(); ++col) {
            (*attenuation)(row, col) =
                distance(row, col) * att_coeff(row, col)
