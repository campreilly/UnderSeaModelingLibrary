/**
 * @file profile_mackenzie_mt.h
 * Multi-threaded Mackenzie model for the speed of sound vs. temperature and salinity.
 */
#ifndef USML_OCEAN_PROFILE_MACKENZIE_MT_H
#define USML_OCEAN_PROFILE_MACKENZIE_MT_H

#include <usml/ocean/profile_grid_mt.h>
...
template< class DATA_TYPE, int NUM_DIMS > class profile_mackenzie_mt
    : public profile_grid_mt<DATA_TYPE,NUM_DIMS>
{

  public:

    /**
     * Multi-Threaded define sound speed profile using temperature and salinity.
     *
     ...
     */
    profile_mackenzie_mt(
        const data_grid<DATA_TYPE,NUM_DIMS>& temperature,
        const data_grid<DATA_TYPE,NUM_DIMS>& salinity,
        attenuation_model* attmodel=NULL)
  	  : profile_grid_mt<DATA_TYPE,NUM_DIMS>(new data_grid<DATA_TYPE,NUM_DIMS>(temperature,false), attmodel)
    {
    	this->m_pclMutex->Lock();
