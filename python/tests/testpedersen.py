...
class TestPedersen(unittest.TestCase):
    ...
    def test_pedersen_compare_shallow(self):
        ...
        grab_direct = usml.eigenrays.grab_eigenrays(filename, 0, 0, 1, 0)
        grab_folded = usml.eigenrays.grab_eigenrays(filename, 1, 0, 0, 0)
        ...
        wq3d_direct = usml.eigenrays.wq3d_eigenrays(filename, 0, 0, 1, 0)
        wq3d_folded = usml.eigenrays.wq3d_eigenrays(filename, 1, 0, 0, 0)
    ...
    def test_pedersen_compare_deep(self):
        ...
        grab_direct = usml.eigenrays.grab_eigenrays(filename, 0, 0, 1, 0, 0.0)
        grab_folded = usml.eigenrays.grab_eigenrays(filename, 0, 0, 1, 0, -90.0)
        filename = os.path.join(self.USML_DIR, "grab_pressure_deep.csv")
        grab_total = np.loadtxt(filename, delimiter=",")

        filename = os.path.join(self.USML_DIR, "pedersen_deep_proploss.nc")
        wq3d_direct = usml.eigenrays.wq3d_eigenrays(filename, 0, 0, 1, 0, 0.0)
        wq3d_folded = usml.eigenrays.wq3d_eigenrays(filename, 0, 0, 1, 0, -90.0)
        wq3d_total = wq3d_proploss(filename)
    ...
    def test_pedersen_sensitivity(self):
        ...
        ax.set_ylabel("Propagation Loss (dB)")
        ax.legend(
            ["Analytic", "$1/80^o$ spacing", "$1/40^o$ spacing", "$1/20^o$ spacing", "$1/10^o$ spacing", "tan spacing"])
        ax.set_ylim(-80, -40)
