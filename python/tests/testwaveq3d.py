...
class TestWaveQ3D(unittest.TestCase):
    ...
    def test_lloyds_spherical(self):
        """Illustrates modeling errors for the spherical equivalent of Lloyd’s mirror,
        ...
        filename = os.path.join(self.USML_DIR, "proploss_lloyds_spherical.nc")
        model = usml.netcdf.EigenrayList(filename)
        filename = os.path.join(self.USML_DIR, "proploss_lloyds_spherical_analytic.nc")
        analytic = usml.netcdf.EigenrayList(filename)
        latitude = analytic.latitude[:, 1] - 45.0
        altitude = analytic.altitude[1, :]
        window = latitude > 0.1
        path_label = ["direct", "surface"]
    ...
    def test_lloyds_spherical_wave(self):
        ...
        filename = os.path.join(self.USML_DIR, "proploss_lloyds_spherical_wave.nc")
        wavefront = usml.netcdf.read(filename)
        src_latitude = wavefront.latitude[0, 0, 0]
