...
//    seq_linear de( -89.0, 0.1, 89.0 ) ;
//    seq_rayfan de( -90.0, 90.0, 361 ) ;
    seq_linear az( 0.0, 2.0, 50.0 ) ;
    ...
    const char* ncname_wave = USML_STUDIES_DIR "/analytic_wedge/analytic_wedge_wave.nc";
...
//    const char* ncname_wave = USML_STUDIES_DIR "/analytic_wedge/analytic_wedge_cxslope_wave.nc";
...
//        target.longitude( n, 0, target.longitude(0,0) + inc * n ) ;
//    }
        // Up slope targets
//    wposition target( num_targets, 1, pos.latitude(), pos.longitude(), -30.0 ) ;
//    double inc = ( 3400.0 * m_2_deg ) / num_targets ;
//    for( int n=1; n < target.size1(); ++n ) {
//        target.latitude( n, 0, -dist + inc * n );
...
//    wave.setIntensityThreshold(100.0) ;
