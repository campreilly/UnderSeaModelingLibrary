 ...
 * 		 https://archive.psas.pdx.edu/CoordinateSystem/, accessed Sept
 * 2023.
 */
class USML_DECLSPEC wvector {
   ...
   public:
     ...
     */
    inline const matrix<double>& rho() const { return _rho; }
     ...
     */
    template <class E>
    inline void rho(const matrix_expression<E>& r, bool no_alias = true) {
        if (no_alias) {
            noalias(_rho) = r;
        } else {
            _rho = r;
     ...
     * @return          Radial coordinate in meters.
     */
    inline double rho(size_t row, size_t col) const { return _rho(row, col); }
     ...
     *
     */
    inline void rho(size_t row, size_t col, double r) { _rho(row, col) = r; }
   ...
   public:
     ...
     */
    inline const matrix<double>& theta() const { return _theta; }
     ...
     */
    template <class E>
    inline void theta(const matrix_expression<E>& t, bool no_alias = true) {
        if (no_alias) {
            noalias(_theta) = t;
        } else {
            _theta = t;
   ...
   public:
     ...
     */
    inline const matrix<double>& phi() const { return _phi; }
     ...
     */
    template <class E>
    inline void phi(const matrix_expression<E>& p, bool no_alias = true) {
        if (no_alias) {
            noalias(_phi) = p;
        } else {
            _phi = p;
     ...
     * @return          Longitude coordinate in radians.
     */
    inline double phi(size_t row, size_t col) const { return _phi(row, col); }
     ...
     *
     */
    inline void phi(size_t row, size_t col, double p) { _phi(row, col) = p; }
     ...
     */
    inline size_t size1() const { return _rho.size1(); }

    /**
     * Number of columns in each coordinate.
     */
    inline size_t size2() const { return _rho.size2(); }
