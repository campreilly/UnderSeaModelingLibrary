%%
% Interactive GUI used to analyze proploss_lloyds_depth
% Illustrates the effect of surface reflection on wavefront folds.

function proploss_lloyds_depth_raytrace
    ...
    wavefront = load_netcdf_wavefront( 'proploss_lloyds_depth_wave.nc' ) ;
    time = 68 ;
    t_index = 1:time ;
    wave_max_time = length(wavefront.travel_time) ;
    proploss = load_netcdf_proploss('proploss_lloyds_depth.nc') ;
    
    %% Figure properties
    grey = [0.8 0.8 0.8] ;
    f = figure('units', 'pixels', ...
               'name', 'Proploss Lloyds Depth Wavefront Propagation', ...
