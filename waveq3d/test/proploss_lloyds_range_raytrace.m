...
function proploss_lloyds_range_raytrace
    ...
    wavefront = load_netcdf_wavefront( 'proploss_lloyds_range_wave.nc' ) ;
    wave_max_time = length(wavefront.travel_time) ;
    time = floor(wave_max_time/2.0) ;
    t_index = 1:time ;
    proploss = load_netcdf_proploss('proploss_lloyds_range.nc') ;
    
    %% Figure properties
    grey = [0.8 0.8 0.8] ;
    f = figure('units', 'pixels', ...
               'name', 'Proploss Lloyds Range Wavefront Propagation', ...
